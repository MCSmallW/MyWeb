...
layui.use(['form','jquery',"layer"],function() {
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['知道了'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">愉快的学习吧？</span></p></div>',
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="../Image/Face/UserFace.jpg" class="userAvatar"/></div>'+
                            '<div class="admin-header-lock-name" id="lockUserName">HJH</div>'+
                            '<div class="input_btn">'+
                                '<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />'+
                                '<button class="layui-btn" id="unlock">解锁</button>'+
                            '</div>'+
                            '<p>密码:“123456”，暂时是静态的</p>'+
                        '</div>',
            closeBtn : 0,
            shade : 0.9,
            success : function(){
        ...
        layer.open({
            ...
            success : function(index, layero){
                ...
                form.on('switch(multipleLogin)', function(data){
                    layer.tips('温馨提示：没做完，请无视', data.othis,{tips: 1})
                });
                form.render();  //表单渲染
            }
        })
            ...
            content : '<div class="skins_box">'+
                            '<form class="layui-form">'+
                                ...
                                '<div class="layui-form-item skinBtn">'+
                                    ...
                                    '<a href="javascript:;" class="layui-btn layui-btn-sm layui-btn-primary" lay-submit="" lay-filter="noChangeSkin">才不要更换咧</a>'+
                                '</div>'+
                            '</form>'+
                        '</div>',
            success : function(index, layero){
